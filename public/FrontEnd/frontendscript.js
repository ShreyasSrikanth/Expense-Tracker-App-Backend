...
async function fetchExpense(currentFormattedDate){
    try {
        ...
        expDropdownSelect.addEventListener('change', async (event) => {
            ...
            await axios.get(`http://54.88.3.81:4000/expense/fetchexpense/${currentFormattedDate}?start=${start}&limit=${limit}`, {
            headers: {
                'Authorization': token
            }
        }).then(res=>{
        ...
        const response = await axios.get(`http://54.88.3.81:4000/expense/fetchexpense/${currentFormattedDate}?start=${start}&limit=${limit}`, {
            headers: {
                'Authorization': token
            }
        });
...
function displayExpenses(expenses) {
    ...
    expenses.forEach((expense) => {
        ...
        deleteButton.addEventListener('click', async () => {
            ...
            const response = await axios.post('http://54.88.3.81:4000/expense/deleteexpense', {
                ID: expense.id,
...
expenseForm.addEventListener('submit', async (event) => {
    ...
    var token = localStorage.getItem('token');
    const response = await axios.post('http://54.88.3.81:4000/expense/storeexpense', {
        category: expenseCategory,
        amount: expenseAmount,
        desc: expenseDescription,
        completed: false
...
if(ispremiumuser ==='true'){
    downloadButton.addEventListener('click', async(e) =>{
        ...
        try {
            var token = localStorage.getItem('token')
            const response = await axios.get(`http://54.88.3.81:4000/expense/download`, {
                headers: {
                    'Authorization': token
                }
            });
            ...
            const allUrls = await axios.get(`http://54.88.3.81:4000/expense/urls`, {
                headers: {
                    'Authorization': token
                }
            });
